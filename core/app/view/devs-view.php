...
if(Core::$user->kind==3){
$products = SellData::getAllBySQL(" where user_id=Core::$user->id and operation_type_id=5 and is_draft=0 order by created_at desc");

}
else if(Core::$user->kind==2){
$products = SellData::getAllBySQL(" where operation_type_id=5 and is_draft=0 and stock_to_id=Core::$user->stock_id order by created_at desc");
}
else{
...
$products = SellData::getAllBySQL(" where person_id=$_SESSION[client_id] and operation_type_id=5 and is_draft=0 order by created_at desc");	
}
...
<h3 class="box-title">Devoluciones</h3></div>
<div class="box-body">
...
doc.save('devolutions-<?php echo date("d-m-Y h:i:s",time()); ?>.pdf');
}
<?php else:?>
doc.save('devolutions-<?php echo date("d-m-Y h:i:s",time()); ?>.pdf');
<?php endif; ?>
